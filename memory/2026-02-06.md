...
## ✅ WhatsApp DM Sender Attribution Bug — FIXED
...
1. **Chat ID ≠ Sender** — Conversation identifier ≠ message author. DMs are keyed by the other party, but each message has its own sender.
2. **Layered bugs** — First fix (allowFrom) revealed second bug (sender attribution). Don't stop at first success.
3. **Trace full data flow** — Baileys → monitor.ts → message-line.ts → envelope.ts. Bug was in the middle layer.
4. **"Already fixed" traps** — Someone added the correct `senderE164` logic in monitor.ts, but downstream code wasn't using it.
5. **Multi-party testing required** — Self-chat tests pass but don't reveal sender confusion bugs.
...
## WhatsApp Inbound Messages Not Working — Root Cause & Fix
...
### Root Cause
1. **Stale WhatsApp Web session** — The session was invalid (status 401) but Baileys reported as "connected"
2. **After QR relink** — Connection re-established but the inbound message listener didn't restart
3. **Bug in `monitor.ts`** — When `loggedOut: true`, the function exits completely instead of waiting for re-authentication
...
### Files Changed
- `/home/globalcaos/src/clawdbot-moltbot-openclaw/src/web/auto-reply/monitor.ts` — Listener restart logic
- `~/.openclaw/openclaw.json` — Added `channels.whatsapp.triggerPrefix: "Jarvis"`
- `/home/globalcaos/src/clawdbot-moltbot-openclaw/src/auto-reply/reply/dispatch-from-config.ts` — Fixed triggerPrefix to use RawBody

### Abstract Principle
**Connection ≠ Functionality** — A socket reporting "open" doesn't mean events are flowing. Always check actual throughput metrics (messagesHandled) not just connection state.

---

## SIGUSR1 Hot-Reload Bug — CRITICAL DISCOVERY (19:49)
...
## ⚠️ CONFIRMED LIMITATION: Outbound DM Messages Not Reaching Baileys (20:23)
...
- ✅ Message appears on his phone
- ✅ Message gets delivered to the recipient
- ✅ Message shows on WhatsApp Web UI (visual sync)
- ❌ Baileys `messages.upsert` event does NOT fire
...
|----------|-----------|-------------------|-----|
| Self-chat (Oscar → Oscar) | Oscar's phone | ✅ Yes | Oscar is the RECIPIENT, so message comes TO his devices |
| DM (Oscar → Sister) | Oscar's phone | ❌ No | Oscar is the SENDER; phone doesn't push to companion devices in real-time |
| DM (Sister → Oscar) | Sister's phone | ✅ Yes | Oscar is the RECIPIENT |
| Group (Oscar sends) | Oscar's phone | ❌ No | Same as DM - sender doesn't push to own devices |
| Group (others send) | Others' phones | ✅ Yes | Oscar is receiving |
...
|--------|--------|-------------|-------|
| **Self-Chat** | Send "Jarvis X" to Saved Messages | ✅ 100% | Limited to self-chat |
| **WhatsApp Web** | Use browser at web.whatsapp.com | ✅ 100% | Messages sent BY Baileys are tracked |
| **History Sync** | Poll `messaging-history.set` event | 🟡 Async | Messages arrive delayed, not real-time |
| **Private Group** | Create group with just yourself | 🟡 May work | Groups sometimes sync better |
| **Relink Device** | Unlink + re-scan QR | 🟡 May fix | Temporary fix at best |
...
## ✅ BUG FIX: senderE164 Resolution for Outbound DMs (20:35)
...
### Files Changed
- `/home/globalcaos/src/clawdbot-moltbot-openclaw/src/web/inbound/monitor.ts` — senderE164 fix

---

## 📦 WhatsApp Skill Published to ClawHub (20:34)
...
## 🤖 AI-to-AI Communication Setup (20:35)
...
| Task | Status |
|------|--------|
| WhatsApp inbound debugging | ✅ Root cause found |
| senderE164 bug fix | ✅ Applied and verified |
| WhatsApp skill published | ✅ v1.0.1 on ClawHub |
| Max (Zeneida's AI) setup | ✅ Whitelisted + instructions |
| Documentation | ✅ Updated |

### Key Learnings
1. **WhatsApp sync limitation is real** — outbound messages from phone don't reliably sync to companion devices
2. **But we had a compounding bug** — senderE164 was wrong for outbound messages
3. **Self-chat works** because you're both sender AND recipient
4. **Fix enables outbound DM triggering** — now Oscar's messages in any DM are correctly attributed to him
