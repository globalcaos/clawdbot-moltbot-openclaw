...
We propose **HIPPOCAMPUS** — a pre-computed concept-to-memory-address index mapping anchor terms to pre-ranked chunk lists offline, enabling **O(1) in corpus size n** inference-time retrieval (O(|A| + K·|A|) work for |A| detected anchors, plus re-ranking over ≤K·|A| candidates). The architecture is inspired by the neuroscientific role of the biological hippocampus as an indexing structure — not a storage structure — between long-term cortical memory and working memory. HIPPOCAMPUS closes the missing architectural layer between ENGRAM (persistent storage) and CORTEX (agent identity).
...
**Theorem 1.** Fix a query distribution $q \sim \mathcal{D}$, and let $m^*$ denote a randomly drawn ground-truth relevant chunk for $q$. Define $P(s)=\Pr_{q\sim\mathcal{D},\,m^*}[s(m^*)=s]$, i.e., the probability mass of _relevant_ chunks that reside in each source. A retrieval strategy that queries a subset $\mathcal{Q} \subseteq S$ of sources achieves expected recall bounded above by:
