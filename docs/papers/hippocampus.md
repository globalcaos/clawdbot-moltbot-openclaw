...
**Theorem 1 (Source-coverage upper bound).** Fix a query distribution $q \sim \mathcal{D}$, and let $m^*$ denote a randomly drawn ground-truth relevant chunk for $q$ (one target chunk per query). Define $P(s)=\Pr_{q\sim\mathcal{D},\,m^*}[s(m^*)=s]$, i.e., the probability mass of _relevant_ chunks that reside in each source. Consider any retrieval strategy whose returned set is restricted to chunks from a subset of sources $\mathcal{Q} \subseteq S$ (it never returns chunks from $S \setminus \mathcal{Q}$). Then the expected Recall@∞ satisfies:
...
**Conclusion:** Theorem 1 accurately predicts recall bounds for any source subset. For partial-coverage source subsets (the first four rows), the average absolute gap between observed recall and the bound is 0.0125. At full coverage, the bound is intentionally loose (gap 0.15), because the dominant errors shift from _source blindness_ to _anchor/ranking_ misses.

**Comparative Benchmarks:** Full benchmark definitions and the canonical results table are reported in Section 7.4 (single source of truth). We keep this section focused on validating Theorem 1's source-coverage bound. The systems evaluated:
...
**Irrelevant Injection Rate (IIR):** Proportion of injected chunks rated irrelevant to the query. Report (i) two-human-annotator IIR (0/1 relevance rubric, ties adjudicated) and (ii) a fixed judge-model IIR for scale (report both; do not mix judge models). Target: IIR < 0.20 at K=20.
...
| Variant                                                            | CORF-Recall@20 | Latency (p95, ms) | Irrelevant injection rate |
| ------------------------------------------------------------------ | -------------: | ----------------: | ------------------------: |
| A1 — HIPPOCAMPUS w/o pronoun expansion                             |         [FILL] |            [FILL] |                    [FILL] |
| A2 — HIPPOCAMPUS w/o post-retrieval re-ranking                     |         [FILL] |            [FILL] |                    [FILL] |
| A3 — HIPPOCAMPUS nightly-only (disable real-time episodic updates) |         [FILL] |            [FILL] |                    [FILL] |
...
- **Code:** Release a reference implementation of index build + lookup (sleep consolidation, real-time updates, re-ranking).
- **Data:** Release an anonymized CORF query set with chunk IDs and source labels, plus annotation guidelines.
- **Compute:** Report hardware (CPU/GPU/RAM), embedding model, batching strategy, and whether embeddings are local or API.
